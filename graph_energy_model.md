...
This layer adds an AST-GNN regression pipeline on top of the Tree-sitter graph builder.
...
- Compares the graph model against the baseline RandomForest predictor on the same test split.

## Run the demo

```bash
python tests/graph_energy_model_demo.py
```

## Run a longer experiment

```bash
python graph_energy_model.py --epochs 12 --limit 200 --batch-size 8
```
